...
Extracted directly from `GrabSpace.esproj`:
...
```
GrabSpace-AR/
├── GrabSpace.esproj      # Lens Studio project file (scene, metadata, platform config)
├── Assets/             # 3D shape asset(s), materials, textures
├── Scripts/            # Hand-tracking + grab/move interaction logic
└── README.md           # Project documentation
```
> Note: only `GrabSpace.esproj` was available at the time of writing; the `Assets/` and `Scripts/` folders above reflect the expected structure as the project is built out.
...
1. Open `GrapSpace.esproj` in **Lens Studio 5.23+**.
